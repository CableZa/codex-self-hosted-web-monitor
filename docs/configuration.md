...
| `UPDATE_STATUS_PATH` | `/runtime/update-status.json` in Docker | Update status file shown by `/api/update-status`. |
| `UPDATE_CHECK_ENABLED` | `true` | Enables passive release checks from the scanner. |
| `UPDATE_CHECK_INTERVAL_SECONDS` | `21600` | Seconds between passive release checks. |
| `UPDATE_CHECK_TIMEOUT_SECONDS` | `8` | Timeout for the release tag HTTP request. |
| `UPDATE_CHECK_TAGS_URL` | GitHub tags API for this repo | JSON tag source used by passive checks. Override this for forks or mirrors. |
| `UPDATE_INSTALL_MODE` | `docker` in Compose, `local` in Windows local scripts | Controls the manual update command shown in the dashboard. |
...
- `runtime/update-status.json`: generated by the scanner or `scripts/update-monitor.py`, ignored by git.
